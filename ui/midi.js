...
// { name, bpm, chords:[{root,q,bass?,beats?}] } → C++ へ渡す形式
function toRequest({name,bpm,chords}){
  return {name,bpm,chords:chords.map(ch=>({root:ch.root,iv:CHORD[ch.q].iv,beats:ch.beats??4,...(ch.bass!=null?{bass:ch.bass}:{})}))};
}
