...
2. **鍵盤／五線譜**：パネル内で切替。鍵盤は3オクターブ（C3〜C6）で、押すとその音を試聴（進行の試聴は止めない）。ルート／スケール音／選択中コードの構成音を色分け。「音名／度数」切替。
   ...
   - 「コード判別」オンの間は、鍵盤・指板のクリックで音を選択／解除（最大8音、紫で表示。指板は1本の弦に1音までで、同じ弦の別フレットを押すと置き換え。鍵盤で選んだ音は指板に薄い紫で表示）し、構成音からコード候補を表示（最低音をベースとして分数コード、5度省略も候補）。候補はクリックで試聴、ドラッグでMIDI。
...
3. **ギター指板**：レギュラーチューニング、0〜15フレット、上が1弦。表示ルールは鍵盤と同じ。押すとその音（1弦開放＝E4〜6弦開放＝E2）を試聴。
   - 「指板｜TAB」切替。TAB はコードを選んでいるとその3ポジション（6弦・5弦・4弦ルートの一番良い形）、選んでいない／試聴中は進行を「ロー／ミドル／ハイ」のエリアで表示（前のコードから手の移動が少ない形）。列を押すとその形を試聴。
